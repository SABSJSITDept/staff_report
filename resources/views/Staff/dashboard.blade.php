...
{{-- IT Pending Recharges Alert --}}
@if(isset($pendingRecharges) && $pendingRecharges->count() > 0)
<div class="mb-6">
            ...
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-red-800 font-bold text-lg flex items-center gap-2">
                        Action Required: Upcoming Bill Payments
                        @php $autoPayCount = $pendingRecharges->where('payment_type', 'Auto Pay')->count(); @endphp
                        @if($autoPayCount > 0)
                            <span class="text-[10px] bg-red-200 text-red-800 px-2 py-0.5 rounded-full font-bold uppercase">{{ $autoPayCount }} Auto Pay</span>
                        @endif
                    </h3>
                    <p class="text-red-700 text-sm">There are {{ $pendingRecharges->count() }} bills due within the next 7 days.</p>
                </div>
            </div>
            <a href="{{ route('it-management.recharges.index') }}" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-lg shadow transition">
                View Bills
            </a>
        </div>
    </div>
