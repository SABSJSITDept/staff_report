            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($recharges as $recharge)
                        @php
                            $isOverdue = \Carbon\Carbon::parse($recharge->last_date)->lt(now());
                            $isDueSoon = !$isOverdue && \Carbon\Carbon::parse($recharge->last_date)->lte(now()->addDays(7));
                        @endphp
                        <tr class="hover:bg-slate-50/50 transition-colors {{ $isOverdue ? 'bg-red-50/30' : ($isDueSoon ? 'bg-amber-50/30' : '') }}">
                        ...
                        <div id="markPaidModal{{ $recharge->id }}" class="fixed inset-0 z-50 hidden">
                            ...
                            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
                                <div class="relative bg-white rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg w-full border border-slate-200">
                                    ...
                                    <form action="{{ route('it-management.recharges.mark-paid', $recharge->id) }}" method="POST" class="p-6">
                                        @csrf
                                        ...
                                        <div class="space-y-4">
                                            <div>
                                                ...
                                                <input type="number" step="0.01" name="amount_paid" value="{{ $recharge->amount }}" required
