...
<div class="max-w-4xl mx-auto animate-fade-in">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Add New Recharge</h1>
            <p class="text-sm text-slate-500">Create a new IT recharge or bill.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <form action="{{ route('it-management.recharges.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="duration_months" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-sm font-medium">
                        <option value="1" {{ old('duration_months') == 1 ? 'selected' : '' }}>Monthly</option>
                        <option value="3" {{ old('duration_months') == 3 ? 'selected' : '' }}>Quarterly</option>
                        <option value="6" {{ old('duration_months') == 6 ? 'selected' : '' }}>Half-Yearly</option>
                        <option value="12" {{ old('duration_months') == 12 ? 'selected' : '' }}>Yearly</option>
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <select name="payment_type" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-sm font-medium">
                        <option value="Manual" {{ old('payment_type') == 'Manual' ? 'selected' : '' }}>Manual</option>
                        <option value="Auto Pay" {{ old('payment_type') == 'Auto Pay' ? 'selected' : '' }}>Auto Pay</option>
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <select name="billing_day" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-sm font-medium">
                        @for($i=1; $i<=31; $i++)
                            <option value="{{ $i }}" {{ old('billing_day') == $i ? 'selected' : '' }}>{{ $i }}{{ in_array($i, [1, 21, 31]) ? 'st' : (in_array($i, [2, 22]) ? 'nd' : (in_array($i, [3, 23]) ? 'rd' : 'th')) }} of the month</option>
                        @endfor
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="mode" value="{{ old('mode') }}" placeholder="e.g. Credit Card, UPI"
                           class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-sm font-medium">
            ...
            <div class="space-y-2">
                ...
                <textarea name="details" rows="3"
                          class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-sm font-medium resize-none leading-relaxed">{{ old('details') }}</textarea>
